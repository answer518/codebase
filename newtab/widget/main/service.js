...
Grid.prototype = (function() {
    ...
    function showDelete(obj) {
        var _this = obj;
        if (!_this.children) {
            Service.onRemoveGrid(_this.index);
        ...
        if (current_drag.children) {
            ...
            forEachGrid(function(item, i) {
                if (i !== current_drag.uiindex && !isCross(current_drag, item)) {
                    ...
                    if (area > moveArea) {
                        moveAnimationTimer();
                        Service.onMovingGrid(current_drag.index, item.index, '');
                        return false;
                    }
                }
            });
            ...
            if (isCross(current_drag, current_drop)) {
                ...
                if (area > moveArea) {
                    moveAnimationTimer();
                    Service.onMovingGrid(current_drag.index, current_drop.index, current_drag.group);
                }
            ...
            if (isOutGroup(xy)) {
                ...
                Service.onMovingGrid(current_drag.index, item.index, current_drag.group);
                return;
            }
            forEachGrid(function(item, i) {
                if (i !== current_drag.uiindex && coverArea(xy, item.getGridFixed()) > moveArea) {
                    moveAnimationTimer();
                    Service.onMovingGroup(current_drag.index, item.index, current_drag.group);
                    return false;
                }
            });
        } else {
        ...
        function stopDrag(event) {
            ...
            if (!current_drop || current_group) {
                ...
                if (isCross(current_drag, current_drop)) {
                    ...
                    Service.onSwappingGrid(current_drag.index, getDropIndex(current_drag, current_drop));
                } else {
                    // fix: top8 不允许合并
                    if (current_drag.isHot === true && current_drop.isHot === true) {
                        ...
                        if (current_drop.children) {
                            ...
                            Service.onMovingInGroup(current_drag.index,
                                list[list.length - (current_drag.uiindex > current_drop.uiindex ? 1 : 1)].index,
                                current_drop.title);
                        } else {
                            // 首次合并
                            Service.onAddGroup(current_drag.index, current_drop.index, Language.getLang('NewFolder'));
                        }
                    }
                }
                current_drop = null;
...
var Service = {
    ...
    initGridDataList: initGridDataList,
